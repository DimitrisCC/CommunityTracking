...
class Muturank_new:
    ...
    def create_sptensors(self, connection):
        ...
        a = {}
        for i, (t, graph) in enumerate(self.graphs.items()):
            irr_graph = self.irr_components(graph)
            # a[i] = sparse.csr_matrix((self.num_of_nodes*self.tfs, self.num_of_nodes*self.tfs), dtype=np.float64)
            a[i] = sparse.eye(self.num_of_nodes * self.tfs, dtype=np.float64, format=Muturank_new.mat_type)
            for u, v, d in irr_graph.edges_iter(data=True):
                # add self edges for nodes that exist
                ...
                # add edges - create symmetric matrix
                try:
                    a[i][i * self.num_of_nodes + self.node_pos[u], i * self.num_of_nodes + self.node_pos[v]] = d[
                        'weight']
                    a[i][i * self.num_of_nodes + self.node_pos[v], i * self.num_of_nodes + self.node_pos[u]] = d[
                        'weight']
                except KeyError:
        ...
        for i in range(self.num_of_nodes * self.tfs):
            self.a_i[i] = hstack(tuple(x.getcol(i) for x in a.values()), format=Muturank_new.mat_type)
        print("Creating o")
        ...
        r = {t: sparse.eye(self.num_of_nodes * self.tfs, dtype=np.float64, format=Muturank_new.mat_type) for t in
             range(self.tfs)}
    ...
    def run_muturank(self):
        ...
        start = True
        while (np.linalg.norm(self.p_new - self.p_old) ** 2 + np.linalg.norm(
                    self.q_new - self.q_old) ** 2 >= self.e) or (
                start):
            start = False
