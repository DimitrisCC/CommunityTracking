...
class dblp_loader:
    ...
    def get_edges(self, start_year, end_year):
        ...
        for year in range(start_year, end_year + 1):
            edge_time[int(year)] = []
            for conf, paper_list in self.data[year].items():
    ...
    def get_conf_graphs(self, conf_edges):
        graphs = {}
        for tf, (year, confs) in enumerate(conf_edges.items()):
            for conf, edges in confs.items():
                try:
    ...
    """def get_comms(self, start_year, end_year):
        ...
        for year in range(start_year, end_year+1):
            # for year, confs in self.data.items():
            comms = {}
            for j, (conf, papers) in enumerate(self.data[year].items(), 1):
    ...
    def get_cc_com(self):
        ...
        conference = {}
        for year, confs in self.conf_graphs.items():
            comms = {}
            conf_map = {}
            com_id = 1
            for conf_name, graph in confs.items():
    ...
    def get_stats(self):
        length = {}
        for year, confs in dblp.data.items():
            for conf, papers in confs.items():
                conf_len = 0
                for paper in papers:
                    conf_len += len(paper)
                if conf_len > 0:
                    try:
                        ...
                        length[conf] = {year: conf_len}
        confs = []
        for name, years in length.items():
            parts_list = [parts for year, parts in years.items() if year in range(2000, 2005)]
    ...
    def create_new_file(self, start_year, end_year, new_file):
        ...
        """
        filtered_data = {}
        for year, conf_dict in self.data.items():
            if year in range(start_year, end_year + 1):
                filtered_data[year] = {}
                for conf, papers in conf_dict.items():
                    if conf in self.conf_list:
    ...
    def get_dynamic_coms(self):
        ...
        for y1, y2 in zip(self.communities.keys(), self.communities.keys()[1:]):
            for id1, com1 in self.communities[y1].items():
                for id2, com2 in self.communities[y2].items():
...
if __name__ == '__main__':
    ...
    for year, confs in dblp.conf_graphs.items():
    ...
    for year, graph in dblp.graphs.items():
        print(year, ", nodes: ", nx.number_of_nodes(graph), ", edges: ", nx.number_of_edges(graph))
        # # TODO:  add some comments
        # conf_life = {}
        # for year, data in dblp.data.items():
        #     if year>1990:
        #         for conf, _ in data.items():
        #             try:
        ...
        # filtered_data = {}
        # for year, conf_dict in dblp.data.items():
        ...
        #         for year, conferences in filtered_data.items():
